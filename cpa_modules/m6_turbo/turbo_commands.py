...
集成方式：
    由M0侵入点3在 pentestagent/interface/commands.py 中自动注册。
"""
...
async def _cmd_turbo_unload(args: list[str], context: Any = None) -> str:
    ...
    # 安全检查：不允许卸载核心模块
    protected = {"sys", "os", "asyncio", "logging", "gc", "cpa_modules"}
    if module_name in protected or module_name.startswith("cpa_modules.m6_turbo"):
