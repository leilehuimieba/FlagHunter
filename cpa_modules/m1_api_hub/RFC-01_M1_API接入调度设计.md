...
```mermaid
flowchart TD
    A[开始 health_check_loop] --> B{监控是否运行?}
    B -->|否| C[退出循环]
    B -->|是| D[获取 ProviderRuntime]
    D --> E{status == DOWN?}
    E -->|是| F[跳过健康检查，等待恢复循环]
    E -->|否| G[_probe 发送探测请求]
    G --> H{探测成功?}
    H -->|是| I[consecutive_failures = 0]
    I --> J[更新 last_success_time]
    J --> K{status != HEALTHY?}
    K -->|是| L[mark_status HEALTHY]
    K -->|否| M[保持 HEALTHY]
    H -->|否| N[consecutive_failures++]
    N --> O[更新 last_failure_time & reason]
    O --> P{consecutive_failures >= threshold?}
    P -->|是| Q[mark_status DOWN]
    Q --> R[触发故障转移]
    P -->|否| S[保持当前状态]
    R --> T[等待 interval 秒]
    M --> T
...
```mermaid
flowchart TD
    A[主 Provider 被标记 DOWN] --> B[调用 failover]
    B --> C[获取所有 HEALTHY Provider]
    C --> D{存在 HEALTHY Provider?}
    D -->|否| E[告警：所有 Provider 不可用]
    E --> F[进入降级模式：缓存响应/重试]
    D -->|是| G[按 priority 排序]
    G --> H[选择优先级最高的备用 Provider]
    H --> I[通知 CostTracker 切换事件]
    I --> J[记录 failover 日志]
    J --> K[更新 TUI 显示]
    K --> L[后续请求路由到新 Provider]
```
...
flowchart TD
    ...
    D -->|是| F[_probe 发送探测请求]
    F --> G{探测成功?}
    G -->|否| H[consecutive_successes = 0]
    H --> E
    G -->|是| I[consecutive_successes++]
    I --> J{consecutive_successes >= 2?}
    J -->|否| E
    J -->|是| K[_verify_with_real_request]
    K --> L{真实请求成功?}
    L -->|否| M[consecutive_successes = 0]
    M --> E
    L -->|是| N[mark_status HEALTHY]
    N --> O[记录恢复日志]
    O --> P[通知 CostTracker]
    P --> E
```
...
# === CPA M1 注入开始 ===
if os.getenv("CPA_M1_API_HUB", "").lower() == "true":
    from cpa_modules.m1_api_hub import shutdown_m1
...
def init_commands():
    ...
    # === CPA M1 注入开始 ===
    if os.getenv("CPA_M1_API_HUB", "").lower() == "true":
        from cpa_modules.m1_api_hub.commands import init_commands as m1_init_cmds
        m1_init_cmds()
    # === CPA M1 注入结束 ===
```
...
sequenceDiagram
    actor User
    participant M0 as PentestAgent(M0)
    ...
    User->>M0: 输入指令
    M0->>M0: ReAct 循环推理
    M0->>PM: select_for_request()
    PM->>PM: 筛选 HEALTHY Provider
    PM->>PM: 按 priority 排序
    PM->>PM: 检查 RPM/TPM 限制
    PM-->>M0: 返回最佳 Provider
    M0->>LLM: completion(model, api_key, base_url, messages)
    LLM->>API: HTTP POST /chat/completions
    API-->>LLM: JSON Response + Usage
    LLM-->>M0: 返回响应
    M0->>PM: update_after_request(log)
    M0->>CT: record(log)
    CT->>CT: 计算成本
    CT->>CT: 检查预算告警
    M0-->>User: 返回结果
...
```mermaid
sequenceDiagram
    participant FM as FailoverMonitor
    participant PM as ProviderManager
    participant LLM as LiteLLM
    participant API as LLM API

    loop 每 30 秒
        FM->>PM: 获取 ProviderRuntime
        FM->>LLM: 发送探测请求 (max_tokens=1)
        alt 探测成功
            LLM-->>FM: 200 OK
            FM->>PM: consecutive_failures = 0
            opt 状态变更
                FM->>PM: mark_status(HEALTHY)
            end
        else 探测失败
            LLM-->>FM: Error / Timeout
            FM->>PM: consecutive_failures++
            opt 达到阈值
                FM->>PM: mark_status(DOWN)
                FM->>PM: 触发 failover()
            end
        end
    end
...
sequenceDiagram
    ...
    loop 每 60 秒（仅对 DOWN 的 Provider）
        FM->>PM: 获取 DOWN 的 Provider
        FM->>LLM: 发送探测请求
        alt 探测失败
            LLM-->>FM: Error
            FM->>PM: consecutive_successes = 0
        else 探测成功
            LLM-->>FM: 200 OK
            FM->>PM: consecutive_successes++
            opt 达到恢复阈值(2次)
                FM->>M0: 发送真实业务请求验证
                alt 真实请求成功
                    M0-->>FM: 正常响应
...
```mermaid
flowchart TD
    A[主 Provider DOWN] --> B{是否有 HEALTHY 备用?}
    B -->|是| C[按 priority 选择备用 Provider]
    C --> D[记录 failover 事件]
    D --> E[通知 CostTracker]
    E --> F[后续请求路由到备用]
    F --> G[备用 Provider 处理请求]
    B -->|否| H[告警：所有 Provider 不可用]
    H --> I[进入完全降级模式]
    I --> J[返回错误提示给用户]
```
...
sequenceDiagram
    actor User
    participant CLI as PentestAgent CLI
    participant CMD as /api Handler
    participant SD as StatusDisplay
    participant PM as ProviderManager
    participant CT as CostTracker

    User->>CLI: 输入 /api
    CLI->>CMD: 路由到 handle_api_command
    CMD->>SD: render()
    SD->>PM: list_all()
    SD->>CT: get_daily_usage()
    SD->>CT: get_recent_logs(10)
    SD-->>CMD: 返回渲染好的 Table
    CMD-->>CLI: 输出到控制台
    CLI-->>User: 显示彩色状态面板

    User->>CLI: 输入 /api switch deepseek-v3
    CLI->>CMD: 路由到 handle_api_command
    CMD->>PM: 手动调整优先级
    PM-->>CMD: 确认切换
    CMD-->>CLI: 显示切换结果
    CLI-->>User: "已切换到 deepseek-v3"
```
