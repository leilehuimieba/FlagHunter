"""Notifier bridge for UI notifications (lives in the session/ base layer).
...
This module lives in ``session/`` (a base layer) so that agents, tools,
runtime, knowledge and workspaces can import it WITHOUT a reverse dependency
on the ``interface/`` entry-point layer. ``interface/notifier.py`` re-exports
these names as a backwards-compatible shim for the TUI. See H8.
"""
