...
class DockerRuntime(Runtime):
    ...
    async def start(self):
        ...
        try:
            ...
            volumes = {
                str(Path.home() / ".flaghunter"): {
                    "bind": "/root/.flaghunter",
                    "mode": "rw",
                },
                **self.config.volumes,
            }
    ...
    async def browser_action(self, action: str, **kwargs) -> dict:
        ...
        if action == "diagnose":
            return {
                ...
                "note": "DockerRuntime currently exposes lightweight HTML fetch, not a rendered Playwright DOM.",
            }
        ...
        if action == "navigate":
            ...
            result = await self.execute_command(
                "cat /tmp/page.html 2>/dev/null || echo ''"
            )
            return {"forms": _extract_forms_from_html(result.stdout or "", kwargs.get("url", ""))}

        elif action == "get_cookies":
            return {
                "cookies": [],
                "cookie_string": "",
                "note": "DockerRuntime does not maintain browser cookie state in html_fetch_only mode",
            }
