...
# Load .env before any other imports that may depend on env vars
try:
...
def _detect_startup_shell() -> str:
    """Best-effort shell detection for startup diagnostics."""
    if os.name == "nt":
        comspec = os.environ.get("COMSPEC", "")
        comspec_lower = comspec.lower()
        if "powershell" in comspec_lower or "PSModulePath" in os.environ:
            return "powershell"
        if comspec:
...
def _detect_wsl_hint() -> str:
    """Return a concise WSL hint string for startup diagnostics."""
...
def _format_startup_argv() -> str:
    """Render argv as a single command string for logs."""
    if os.name == "nt":
        return subprocess.list2cmdline(sys.argv)
    return shlex.join(sys.argv)


def _log_startup_context(args: argparse.Namespace) -> None:
    """Emit a one-line startup context record for later debugging."""
    logger.info(
        "startup_context command=%s python=%s version=%s cwd=%s shell=%s os=%s "
        "os_release=%s wsl=%s argv=%s",
        getattr(args, "command", None) or "<none>",
        sys.executable,
        sys.version.split()[0],
        os.getcwd(),
        _detect_startup_shell(),
        platform.system(),
        platform.release(),
        _detect_wsl_hint(),
        _format_startup_argv(),
    )
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  flaghunter tui                              Launch TUI
  flaghunter tui -t 192.168.1.1               Launch TUI with target
  flaghunter run -t localhost --task "scan"   Headless run
  flaghunter web                              Launch Web Console (port 8080)
  flaghunter web --port 3000                  Launch Web Console on custom port
  flaghunter tools list                       List available tools
  flaghunter mcp list                         List MCP servers
        """,
    )
    ...
    runtime_parent.add_argument(
        ...
        dest="ssh",
        action="store_true",
        help="Use Kali VM via SSH for tool execution",
    )
    runtime_parent.add_argument(
        "--playbook",
        "-p",
    ...
    run_parser.add_argument(
        "--mode",
        choices=["auto", "pentest", "ctf"],
        default=None,
        help="Task mode override for headless run",
    )
    run_parser.add_argument(
        "--ctf-type",
        dest="ctf_type",
    ...
    run_parser.add_argument(
        ...
        action="append",
        default=[],
        help="Optional local artifact path; can be repeated",
    )
    run_parser.add_argument(
        "--report",
        "-r",
    ...
    mcp_connect.add_argument(
        "--detach",
        action="store_true",
        help="Run as background daemon (writes PID file at ~/.flaghunter/mcp.pid)",
    )
    ...
    # Web console subcommand
    web_parser = subparsers.add_parser(
        "web", help="Launch Web Console (Mission Control) at http://localhost:8080"
    )
    web_parser.add_argument(
        "--host", default="127.0.0.1", help="Bind host (default: 127.0.0.1)"
    )
    web_parser.add_argument(
        "--port", "-p", type=int, default=8080, help="Bind port (default: 8080)"
    )

    ctf_parser = subparsers.add_parser(
        "ctf", help="CTF helper commands"
    )
    ctf_subparsers = ctf_parser.add_subparsers(dest="ctf_command", help="CTF commands")
    ctf_memory = ctf_subparsers.add_parser(
        "memory", help="Manage strategy memory entries"
    )
    ctf_memory.add_argument(
        "action",
        nargs="?",
        default="summary",
        help="summary | list | show | mute | activate | audit",
    )
    ctf_memory.add_argument(
        "rest",
        nargs=argparse.REMAINDER,
        help="Additional arguments for the selected action",
    )
...
def handle_tools_command(args: argparse.Namespace):
    ...
    if args.tools_command == "list":
        ...
        try:
            ...
            mcp_socket_path = Path.home() / ".flaghunter" / "mcp.sock"
        except Exception:
            mcp_socket_path = None
        ...
        console.print("[yellow]Use 'flaghunter tools --help' for commands[/]")
    ...
    if args.tools_command == "call":
        ...
        sock = Path.home() / ".flaghunter" / "mcp.sock"
        if sock.exists():
            try:
                with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as s:
                    s.connect(str(sock))
...
def handle_mcp_command(args: argparse.Namespace):
    ...
    if args.mcp_command == "list":
        ...
        if not servers:
            ...
            console.print(
                "\nAdd a server with: flaghunter mcp add <name> <command> <args...>"
            )
            return
        ...
        console.print("[yellow]Use 'flaghunter mcp --help' for available commands[/]")
...
def handle_workspace_command(args: argparse.Namespace):
    ...
    from flaghunter.workspaces.manager import WorkspaceError, WorkspaceManager
    from flaghunter.workspaces.utils import (
        export_workspace,
        import_workspace,
...
def _format_ctf_memory_entry_brief(entry) -> str:
...
def _format_ctf_memory_entry_detail(entry) -> str:
    return "\n".join(
        [
...
def _parse_ctf_memory_common_options(tokens: list[str]) -> tuple[str | None, str, list[str]]:
    status = None
    sort_by = "recent"
    extras: list[str] = []
    for raw_token in tokens:
        ...
        if lowered in {"active", "muted", "deprecated"}:
            ...
            if candidate in {"active", "muted", "deprecated"}:
                status = candidate
            elif candidate == "all":
                status = None
            else:
            ...
            extras.append(token)
    return status, sort_by, extras


def handle_ctf_command(args: argparse.Namespace):
    """Handle CLI CTF helper commands."""
    from ..agents.pa_agent.strategy_memory import StrategyMemoryStore

    if args.ctf_command != "memory":
        print(
            "Usage:\n"
            "  flaghunter ctf memory\n"
            "  flaghunter ctf memory list [limit] [active|muted|deprecated|filter=<...>] [sort=recent|correlation|applied|last_used]\n"
            "  flaghunter ctf memory show <id>\n"
            "  flaghunter ctf memory mute <id>\n"
            "  flaghunter ctf memory activate <id>\n"
            "  flaghunter ctf memory rollback <id>\n"
            "  flaghunter ctf memory audit [threshold] [sort=correlation|recent|applied|last_used]\n"
            "  flaghunter ctf memory delete <id>\n"
            "  flaghunter ctf memory export <path>\n"
            "  flaghunter ctf memory clear confirm"
        )
        return

    store = StrategyMemoryStore()
    action = str(args.action or "summary").strip().lower()
    ...
    async def _run():
        if action == "summary":
            ...
            if not entries:
                print("[CTF memory] 暂无记忆条目。")
                return
            print("[CTF memory summary]")
            for entry in entries:
                print(_format_ctf_memory_entry_brief(entry))
            return

        if action == "list":
            limit = 10
            status, sort_by, extras = _parse_ctf_memory_common_options(rest)
            for token in extras:
                if token.isdigit():
            ...
            if not entries:
                ...
                return
            print(
                f"[CTF memory list] count={len(entries)}"
                f" filter={status or 'all'} sort={sort_by}"
            )
            for entry in entries:
                print(_format_ctf_memory_entry_brief(entry))
            return
        ...
        if action == "show":
            if not rest:
                print("Usage: flaghunter ctf memory show <id>")
                return
            entry = await store.get_entry(rest[0])
            if entry is None:
                print(f"[CTF memory] 未找到条目: {rest[0]}")
                return
            print(_format_ctf_memory_entry_detail(entry))
            return

        if action == "mute":
            if not rest:
                print("Usage: flaghunter ctf memory mute <id>")
                return
            entry = await store.mute_entry(rest[0])
            if entry is None:
                print(f"[CTF memory] mute 失败: {rest[0]}")
                return
            print(f"[CTF memory] muted {entry.id}")
            print(_format_ctf_memory_entry_brief(entry))
            return

        if action == "activate":
            if not rest:
                print("Usage: flaghunter ctf memory activate <id>")
                return
            entry = await store.activate_entry(rest[0])
            if entry is None:
                print(f"[CTF memory] activate 失败: {rest[0]}")
                return
            print(f"[CTF memory] activated {entry.id}")
            print(_format_ctf_memory_entry_brief(entry))
            return

        if action == "rollback":
            if not rest:
                print("Usage: flaghunter ctf memory rollback <id>")
                return
            entry = await store.rollback_mute(rest[0])
            if entry is None:
                print(f"[CTF memory] rollback 失败: {rest[0]}")
                return
            print(f"[CTF memory] rollback applied to {entry.id}")
            print(_format_ctf_memory_entry_brief(entry))
            return

        if action == "audit":
            threshold = 0.3
            _, sort_by, extras = _parse_ctf_memory_common_options(rest)
            for token in extras:
                try:
            ...
            if not entries:
                ...
                return
            print(
                f"[CTF memory audit] count={len(entries)}"
                f" threshold={threshold:.2f} sort={sort_by}"
            )
            for entry in entries:
                print(_format_ctf_memory_entry_brief(entry))
            return
        ...
        if action == "delete":
            if not rest:
                print("Usage: flaghunter ctf memory delete <id>")
                return
            deleted = await store.delete_entry(rest[0])
            if not deleted:
                print(f"[CTF memory] delete 失败: {rest[0]}")
                return
            print(f"[CTF memory] deleted {rest[0]}")
            return

        if action == "export":
            if not rest:
                print("Usage: flaghunter ctf memory export <path>")
                return
            exported = await store.export_entries(rest[0])
            print(f"[CTF memory] exported to {exported}")
            return

        if action == "clear":
            if not rest or str(rest[0]).strip().lower() != "confirm":
                print("Usage: flaghunter ctf memory clear confirm")
                return
            count = await store.clear_entries()
            print(f"[CTF memory] cleared {count} entries")
            return
...
def main():
    ...
    if args.command is None:
        ...
        run_tui(target=None, model=DEFAULT_MODEL, use_docker=False, use_ssh=False)
        return
