...
class ConversationStore:
    ...
    def save(
        ...
        try:
                ...
                json.dumps(
                    {
                        ...
                        "messages": serialized,
                        "handoff": dict(handoff or {}) if isinstance(handoff, dict) else None,
                    },
                    indent=2,
                ),
                encoding="utf-8",
            ...
            normalized_handoff = dict(handoff or {}) if isinstance(handoff, dict) else {}
            if normalized_handoff:
                entry["last_run_id"] = str(normalized_handoff.get("last_run_id") or "").strip()
                entry["last_checkpoint"] = str(normalized_handoff.get("last_checkpoint") or "").strip()
                entry["last_ledger"] = str(normalized_handoff.get("last_ledger") or "").strip()
                entry["last_resume_from_run"] = str(normalized_handoff.get("last_resume_from_run") or "").strip()
                entry["last_resume_from_checkpoint"] = str(normalized_handoff.get("last_resume_from_checkpoint") or "").strip()
            if is_new:
    ...
    def load_handoff_metadata(self, conv_id: str) -> Optional[dict]:
        """Load the optional handoff metadata saved alongside a conversation."""
        try:
            conv_path = self._get_dir() / f"{conv_id}.json"
            if not conv_path.exists():
                log.warning("Conversation not found: %s", conv_id)
            ...
            log.exception("ConversationStore.load_handoff_metadata failed for %s: %s", conv_id, e)
            return None
