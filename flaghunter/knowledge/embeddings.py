...
# Official OpenAI embedding endpoints — anything else is a relay/gateway
_OFFICIAL_OPENAI_BASES = frozenset({
...
def should_use_local_embeddings() -> bool:
    ...
    if api_base and api_base not in _OFFICIAL_OPENAI_BASES:
        # Custom relay — avoid assuming it supports the embeddings endpoint
        return True
...
def _is_openai_embedding_model(model: str) -> bool:
    ...
    m = model.lower()
    return m.startswith(("text-embedding-", "openai/"))
...
def get_embeddings(
    ...
    try:
        ...
        # Forward api_base when a custom OpenAI-compatible endpoint is configured
        # and the model targets the OpenAI embeddings API.
        api_base = get_openai_api_base()
        if api_base and _is_openai_embedding_model(model):
...
# Module-level cache for the local encoder to avoid re-loading on every call
...
def get_embeddings_local(
    ...
    try:
        ...
        # Suppress noisy unauthenticated-request warnings from huggingface_hub
