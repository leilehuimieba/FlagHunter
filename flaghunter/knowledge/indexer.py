...
def resolve_knowledge_scan_paths(directory: Path) -> list[Path]:
    """Resolve one or more knowledge directories that should be scanned.

    Supports both of these layouts:
    - ``<knowledge>/sources`` plus sibling ``<knowledge>/sessions``
    - direct source directory input such as ``.../knowledge/sources``
    """
    directory = Path(directory)

    if directory == Path("knowledge"):
        kp = resolve_knowledge_paths()
        base = kp.get("base", directory)
        directory = Path(base)
    ...
    if directory.name == "sources":
        ...
        candidates.append(directory.parent / "sessions")
        candidates.append(directory.parent / "ctf_sessions")
        candidates.append(directory.parent / "retrospective_export")
    elif (
        (directory / "sources").exists()
        or (directory / "sessions").exists()
        or (directory / "ctf_sessions").exists()
        or (directory / "retrospective_export").exists()
    ):
        candidates.append(directory / "sources")
        candidates.append(directory / "sessions")
        candidates.append(directory / "ctf_sessions")
        candidates.append(directory / "retrospective_export")
    else:
        candidates.append(directory)
...
class KnowledgeIndexer:
    ...
    def index_directory(self, directory: Path) -> tuple[List[Document], IndexingResult]:
        ...
        scan_roots = resolve_knowledge_scan_paths(directory)
