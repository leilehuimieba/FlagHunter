"""M5 swarm bridge for crew worker completion events and Phase 7 CTF adapters."""

import json
import os
from typing import Any, Awaitable, Callable

# Severity ranking for ordering peer-agent findings in the orchestrator prompt:
# the most actionable (critical/high) surface first so the next dispatch round
# re-prioritises around them. Unknown severities sort last.
_SEVERITY_ORDER = {"critical": 0, "high": 1, "medium": 2, "low": 3, "info": 4}


def _extract_flag_values(state: Any, attr_name: str) -> list[str]:
    bucket = list(getattr(state, attr_name, []) or [])
    values: list[str] = []
    for item in bucket:
        value = str(getattr(item, "value", "") or "").strip()
        if value:
            values.append(value)
    return values


def _extract_state_diff(state: Any) -> dict[str, Any]:
    observations = []
    for item in list(getattr(state, "observations", []) or []):
        observations.append(
            {
                "kind": str(getattr(item, "kind", "") or ""),
                "value": str(getattr(item, "value", "") or ""),
                "source": str(getattr(item, "source", "") or ""),
                "metadata": dict(getattr(item, "metadata", {}) or {}),
            }
        )
    return {
        "observations": observations,
        "candidate_flags": _extract_flag_values(state, "candidate_flags"),
        "runtime_flags": _extract_flag_values(state, "runtime_flags"),
        "verified_flags": _extract_flag_values(state, "verified_flags"),
        "rejected_flags": _extract_flag_values(state, "rejected_flags"),
        "stop_reason": str(getattr(state, "stop_reason", "") or ""),
    }


async def run_ctf_dispatcher_worker(
    dispatcher: Any,
    *,
    target: str,
    goal: str,
    chtype: str = "auto",
    hint: str = "",
    submit_profile: dict[str, Any] | None = None,
    worker_id: str = "",
    worker_type: str = "default",
    cancel_event: Any = None,
) -> dict[str, Any]:
    """Wrap a dispatcher run so crew mode can consume it as a worker result."""
    if cancel_event is not None and getattr(cancel_event, "is_set", lambda: False)():
        return {
            "worker_id": worker_id,
            "worker_type": worker_type,
            "cancelled": True,
            "verified_flag": None,
            "candidate_flags": [],
            "observations": [],
            "state_diff": {},
        }

    run_kwargs = {
        "target": target,
        "goal": goal,
        "type": chtype,
        "hint": hint,
    }
    if submit_profile is not None:
        run_kwargs["submit_profile"] = submit_profile

    solve_result = await dispatcher.run(**run_kwargs)
    state = getattr(dispatcher, "state", None)
    await publish_solve_outcome_to_swarm(
        worker_id or worker_type or "ctf-worker",
        target,
        solve_result,
        state,
    )
    state_diff = _extract_state_diff(state) if state is not None else {}

    return {
        "worker_id": worker_id,
        "worker_type": worker_type,
        "success": bool(getattr(solve_result, "success", False)),
        "reason": str(getattr(solve_result, "reason", "") or ""),
        "missing_tools": list(getattr(solve_result, "missing_tools", []) or []),
        "verified_flag": str(getattr(solve_result, "flag", "") or "") or None,
        "candidate_flags": list(state_diff.get("candidate_flags") or []),
        "runtime_flags": list(state_diff.get("runtime_flags") or []),
        "observations": list(state_diff.get("observations") or []),
        "state_diff": state_diff,
    }


def build_ctf_dispatcher_worker_runner(
    dispatcher_factory: Callable[[], Any],
    *,
    target: str,
    goal: str,
    chtype: str = "auto",
    hint: str = "",
    submit_profile: dict[str, Any] | None = None,
) -> Callable[[dict[str, Any], Any, Any], Awaitable[dict[str, Any]]]:
    """Create a coordinator-compatible worker runner from a dispatcher factory."""

    async def _runner(spec: dict[str, Any], shared_state: Any, cancel_event: Any) -> dict[str, Any]:
        dispatcher = dispatcher_factory()
        return await run_ctf_dispatcher_worker(
            dispatcher,
            target=target,
            goal=goal,
            chtype=chtype,
            hint=hint,
            submit_profile=submit_profile,
            worker_id=str(spec.get("worker_id") or ""),
            worker_type=str(spec.get("worker_type") or "default"),
            cancel_event=cancel_event,
        )

    return _runner


async def publish_solve_outcome_to_swarm(
    agent_id: str,
    target: str,
    solve_result: Any,
    state: Any,
) -> bool:
    """Bridge a single-agent dispatcher outcome into the crew swarm (M5).

    Publishes the single-agent Fact/Intent/Hint blackboard (verified/runtime
    flags) to the shared messenger and deposits *chain* pheromone into the shared
    PheromoneRouter — namespaced ``chain:<name>`` so it coexists with the
    target pheromone. Shorter winning chains deposit more per chain (走最短链),
    so the swarm's chain pheromone converges to short paths just like the
    single-agent strategy memory does.

    Gated on ``CPA_M5_SWARM_LINK`` (off by default → solo runs are unaffected)
    and best-effort (never raises). Returns True when it published. This is the
    *publish* half of the single-agent ⇄ swarm bridge; the consume half is
    :func:`recall_swarm_chain_pheromone`, available for multi-agent biasing.
    """
    if os.getenv("CPA_M5_SWARM_LINK", "false").lower() != "true":
        return False
    try:
        from flaghunter.cpa_modules.m5_swarm_link import get_messenger, get_pheromone_router

        success = bool(getattr(solve_result, "success", False))
        chain_used = [
            str(chain).strip()
            for chain in (getattr(solve_result, "chain_used", []) or [])
            if str(chain).strip()
        ]

        router = get_pheromone_router()
        if success and chain_used:
            per_chain = 1.0 / len(chain_used)
            for chain in dict.fromkeys(chain_used):
                await router.deposit(
                    target=f"chain:{chain}", source=agent_id, strength=per_chain
                )

        if state is not None:
            from ..pa_agent.blackboard import project_blackboard

            messenger = get_messenger(agent_id)
            board = project_blackboard(state)
            for fact in board.get("facts", []):
                kind = str(fact.get("kind") or "")
                if kind not in {"verified_flag", "runtime_flag"}:
                    continue
                await messenger.report_finding(
                    target=target,
                    finding=f"{kind}={fact.get('value')}",
                    severity="high" if kind == "verified_flag" else "info",
                    details={"source": str(fact.get("source") or "")},
                )
        return True
    except Exception:
        return False


async def recall_swarm_chain_pheromone() -> dict[str, float]:
    """Read chain pheromone deposited by peer agents from the shared router.

    Consume half of the single-agent ⇄ swarm bridge: returns
    ``{chain_name: strength}`` for ``chain:``-namespaced trails. Gated on
    ``CPA_M5_SWARM_LINK``; empty when disabled/unavailable. Available for
    multi-agent chain biasing — not yet wired into the solo chain-order bias
    (which uses persisted strategy_memory pheromone), so behaviour is unchanged
    until multi-agent collaboration is turned on.
    """
    if os.getenv("CPA_M5_SWARM_LINK", "false").lower() != "true":
        return {}
    try:
        from flaghunter.cpa_modules.m5_swarm_link import get_pheromone_router

        router = get_pheromone_router()
        trails = await router.get_active_trails()
        pheromone: dict[str, float] = {}
        for trail in trails:
            name = str(getattr(trail, "target", "") or "")
            if name.startswith("chain:"):
                pheromone[name[len("chain:"):]] = float(
                    getattr(trail, "current_strength", 0.0) or 0.0
                )
        return pheromone
    except Exception:
        return {}


def _severity_rank(msg: Any) -> int:
    sev = str((getattr(msg, "metadata", {}) or {}).get("severity", "") or "").lower()
    return _SEVERITY_ORDER.get(sev, 5)


def _finding_summary(msg: Any) -> str:
    """One-line summary of a blackboard ``finding`` message.

    ``report_finding`` stores the payload as a JSON string carrying the canonical
    ``kind=value`` fact text (see ``publish_solve_outcome_to_swarm``); fall back to
    the raw content when it is not JSON.
    """
    raw = str(getattr(msg, "content", "") or "")
    try:
        data = json.loads(raw)
        text = str(data.get("finding") or raw)
    except Exception:
        text = raw
    text = text.strip().replace("\n", " ")
    return text[:160] + ("…" if len(text) > 160 else "")


async def recall_swarm_board_context(
    target: str, *, max_findings: int = 8, max_targets: int = 5
) -> str:
    """Read peer-agent findings + pheromone hot targets from the shared board (M5).

    The *read* half of the orchestrator ⇄ swarm loop. Workers publish facts and
    chain pheromone via :func:`publish_solve_outcome_to_swarm` /
    ``report_finding``; the orchestrator folds this back into its system prompt
    each iteration so the next round of worker dispatch is informed by what peers
    already found ("退而求其次获取信息，下次进攻可以更好"). Findings are ordered
    high-severity first so the model re-prioritises around the hottest signals.

    Gated on ``CPA_M5_SWARM_LINK`` **and** an initialised M5 (the getters raise
    until ``init_m5`` runs); best-effort, never raises. Returns a markdown block,
    or ``""`` when disabled/unavailable/empty — so with the gate off the
    orchestrator prompt is byte-identical to before. M5 itself is unchanged.
    """
    if os.getenv("CPA_M5_SWARM_LINK", "false").lower() != "true":
        return ""
    try:
        from flaghunter.cpa_modules.m5_swarm_link import (
            get_blackboard,
            get_pheromone_router,
            is_m5_enabled,
        )

        if not is_m5_enabled():
            return ""

        board = get_blackboard()
        # Over-fetch then re-rank by severity (query returns newest-first).
        findings = await board.get_by_type("finding", limit=max_findings * 3)
        ranked = sorted(
            findings,
            key=lambda m: (_severity_rank(m), -getattr(m, "timestamp").timestamp()),
        )
        finding_lines: list[str] = []
        for msg in ranked[:max_findings]:
            sev = str((getattr(msg, "metadata", {}) or {}).get("severity", "") or "")
            tag = f"[{sev}] " if sev else ""
            scope = f" — {msg.target}" if getattr(msg, "target", "") else ""
            who = f" ({msg.sender})" if getattr(msg, "sender", "") else ""
            finding_lines.append(f"- {tag}{_finding_summary(msg)}{scope}{who}")

        router = get_pheromone_router()
        top = await router.get_top_targets(n=max_targets)
        # Drop the ``chain:``-namespaced trails (those bias chain order, not target
        # selection); surface only real attack targets as hot.
        hot_lines = [
            f"- {tgt} (强度 {strength:.2f})"
            for tgt, strength in top
            if not str(tgt).startswith("chain:")
        ]

        sections: list[str] = []
        if finding_lines:
            sections.append(
                "### Peer-agent findings (shared blackboard)\n" + "\n".join(finding_lines)
            )
        if hot_lines:
            sections.append("### Hot targets (pheromone)\n" + "\n".join(hot_lines))
        return "\n\n".join(sections)
    except Exception:
        return ""


async def on_worker_complete(
    agent_id: str,
    task: str,
    target: str,
    result: str,
    success: bool,
):
    """Forward worker completion to M5 when enabled."""
    try:
        if os.getenv("CPA_M5_SWARM_LINK", "false").lower() != "true":
            return

        from flaghunter.cpa_modules.m5_swarm_link import get_messenger, get_pheromone_router

        router = get_pheromone_router()
        await router.deposit(
            target=target,
            source=agent_id,
            strength=1.0 if success else 0.2,
        )

        messenger = get_messenger(agent_id)
        await messenger.report_finding(
            target=target,
            finding=(result or "")[:500],
            severity="info",
        )
    except Exception:
        pass
