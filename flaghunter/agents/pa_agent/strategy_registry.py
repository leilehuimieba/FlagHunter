...
def _render_param_precondition(context: StrategyContext) -> bool:
    ...
    dispatcher_state = getattr(context.dispatcher, "state", None)
...
def _ssti_probe_ran(context: StrategyContext) -> bool:
    ...
    state = getattr(context.dispatcher, "state", None)
    if state is None:
        return False
    probe_ran = False
    probe_hit = False
...
__all__ = ["StrategyContext", "StrategyDefinition", "StrategyRegistry"]
