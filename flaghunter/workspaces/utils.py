...
def _get_active_workspace_name(root: Path) -> str:
    """Return the active workspace name without importing WorkspaceManager.

    This keeps lightweight consumers such as notes/CTF dispatcher usable even
    when optional workspace metadata dependencies are not installed.
    """
    marker = root / "workspaces" / ".active"
...
def get_conversations_base(root: Optional[Path] = None) -> Path:
    ...
    global _WARNED_CONV
    root = Path(root or "./")
    active = _get_active_workspace_name(root)
    if active:
