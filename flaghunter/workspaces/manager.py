...
class TargetManager:
    ...
    def normalize_host(value: str) -> str:
        """Normalize a bare host/IP value for scope comparison.

        All loopback aliases are canonicalized to ``127.0.0.1`` so that
        ``localhost``, ``127.0.0.1`` and ``::1`` are treated as the same
        sandbox target during local CTF work.
        """
        ...
        try:
            ...
            if TargetManager.HOST_RE.match(host) and ".." not in host:
                return host
            raise WorkspaceError(f"Invalid target: {value}") from None

    @staticmethod
    def _format_url_host(host: str) -> str:
        """Format a normalized host for URL netloc reconstruction."""
        try:
            ip = ipaddress.ip_address(host)
            if ip.version == 6:
                return f"[{host}]"
    ...
    def normalize_target(value: str) -> str:
        ...
        if parsed.scheme and parsed.hostname:
            ...
            netloc = TargetManager._format_url_host(host)
        ...
        try:
            ...
            return TargetManager.normalize_host(v)
