...
随着 Stage I ~ Stage III 的实际推进，项目**已经落地成功**，但实际代码实现采用了更小、更直接的方案，因此：
...
| `前后端联调与验收清单_V1` | **Stage I ~ III 联调目标大体已完成，可作为历史验收 checklist 参考** |
...
6. Settings get
7. 事件流接口
8. 任务动作：创建 / 停止 / hint / 附件
9. 结构化 SSE 事件（至少 `tool.finished / knowledge.retrieved / note.created`）
...
1. Settings 持久化写入
2. 更强的 run / session 精确匹配
3. richer trace graph data model
4. 更完整的 knowledge usage 统计支撑
...
| Settings 可写化 | 未完成 | 当前仍只读 |
| Trace Graph 真图化 | 未完成 | 当前 live 仍 `notReady` |
| Task replay 精细化 | 未完成 | 当前仍存在 snapshot/fallback 风险 |
| Knowledge 高阶分析 | 部分完成 | 基础统计已通，增强分析未做 |
...
> **这批规划文档已经完成了“把项目带到正确方向”的使命，但它们现在更像历史设计参考，而不是当前实现真相；当前实现真相必须以 Stage I~III 总交接 V2、状态矩阵和当前代码为准。**
