...
- `ctf_dispatcher` 结构化 follow-up 选主
  当前已不只看 `nextAction`，还会结合 `switchedFrom / triggerReason / triggerActionDriver` 把 `collect_initial_facts` 这类退化 follow-up 继续拉回高价值策略，例如 `ssti_exploit`、`hash_guarded_file_read` 与 `backup_source_leak`
