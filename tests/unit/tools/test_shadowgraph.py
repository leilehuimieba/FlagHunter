...
from pentestagent.tools.shadowgraph import shadowgraph


class TestShadowgraph:
    @patch("pentestagent.tools.shadowgraph.get_all_notes_sync")
    ...
    @patch("pentestagent.tools.shadowgraph.get_all_notes_sync")
    @patch("pentestagent.tools.shadowgraph.ShadowGraph")
    ...
    def test_default_action_is_insights(self, mock_get_notes):
        mock_get_notes.return_value = {"note1": {"host": "192.168.1.1"}}
        with patch("pentestagent.tools.shadowgraph.ShadowGraph") as mock_graph_cls:
            mock_graph = MagicMock()
