...
def test_playbook_records_state_store_adapter_production_wiring_landing() -> None:
    text = _playbook_text()
    section = _heading_section_text(
        text,
    ...
    assert "Status: state-store adapter production wiring landed." in section
    assert "approved scope: session composition-root snapshot-service wiring only" in section
    assert "`flaghunter/session/initializer.py::build_challenge_snapshot_service`" in section
    assert "`StateStoreAdapter` wraps the injected `StateStorePort`" in section
    assert "`BuildChallengeRunSnapshot` still depends only on ports" in section
    assert "test_build_challenge_snapshot_service_wires_state_store_adapter" in section
    assert "test_p1_state_store_adapter_is_wired_only_in_session_composition_root" in section
    for boundary in (
...
def test_playbook_records_state_ownership_snapshot_seam_implementation_record() -> None:
    text = _playbook_text()
    section = _heading_section_text(
        text,
    ...
    assert "Status: snapshot seam first slice implemented in the working tree." in section
    assert "Approved user message: 批准 State ownership split 第一刀: snapshot ownership seam" in section
    assert "`CTFState.to_snapshot` delegates to `_export_state_snapshot`" in section
    assert "`CTFState.from_snapshot` delegates to `_restore_state_snapshot`" in section
    assert "claim-store ownership migration remains unstarted" in section
    ...
    for expected in (
        ...
        "AttributeError: module 'flaghunter.agents.pa_agent.ctf_state' has no attribute '_export_state_snapshot'",
...
def test_playbook_records_state_ownership_seam_aggregate_transition_guard() -> None:
    text = _playbook_text()
    section = _heading_section_text(
        text,
    ...
    assert "Status: seam aggregate guard recorded; production store wiring still blocked." in section
    assert "Approved user message: 批准 State ownership split 第三刀: seam aggregate transition guard" in section
    rows = {
        row["State surface"]: row
        for row in _markdown_table_rows(section)
    }
    expected_rows = {
        "snapshot ownership seam": {
            ...
            "Counts as production store wiring": "false",
        },
        "claim-store ownership seam": {
        ...
        "state-store adapter production wiring": {
            "Required evidence": "future explicit State store wiring approval",
            "Current state": "blocked",
            "Counts as production store wiring": "true",
        },
        "claim-store adapter production wiring": {
            "Required evidence": "future explicit Claim store wiring approval",
            "Current state": "blocked",
            "Counts as production store wiring": "true",
        },
        "composition root state wiring": {
...
def test_playbook_records_state_store_adapter_production_wiring_characterization() -> None:
    text = _playbook_text()
    section = _heading_section_text(
        text,
    ...
    assert "Status: characterization recorded; state-store adapter production wiring not approved." in section
    assert "Approved user message: 批准 State ownership split 第四刀: state-store adapter production wiring characterization" in section
    rows = {
        row["Surface"]: row
        for row in _markdown_table_rows(section)
    }
    expected_rows = {
        "state-store port application-service injection": {
            "Current state": "existing skeleton",
            "Required guard": "`BuildChallengeRunSnapshot` accepts injected `StateStorePort` only",
            "Production wiring approved": "false",
        },
        "StateStoreAdapter import surface": {
            "Current state": "guarded unwired",
            "Required guard": "`test_p1_state_store_adapter_stays_unwired_from_production_imports`",
            "Production wiring approved": "false",
        },
        "StateStoreAdapter delegate body": {
            "Current state": "direct delegate only",
            "Required guard": "`test_state_store_adapter_action_bodies_remain_direct_delegate_only`",
            "Production wiring approved": "false",
        },
        "CTFState store dependency": {
            "Current state": "unwired",
            "Required guard": "`test_p1_ctf_state_stays_unwired_from_state_and_claim_store_ports`",
            "Production wiring approved": "false",
        },
        "composition root state-store binding": {
            "Current state": "blocked",
...
def test_playbook_records_state_ownership_production_wiring_closeout_next_gate() -> None:
    text = _playbook_text()
    section = _heading_section_text(
        text,
    ...
    assert "Status: production wiring closeout recorded; composition-root binding approval still required." in section
    assert "Approved user message: 批准 State ownership split 第九刀" in section
    rows = {
        row["State ownership surface"]: row
        for row in _markdown_table_rows(section)
    }
    expected_rows = {
        "state-store adapter production wiring": {
            "Required evidence": "`State store adapter production wiring landing record`",
            "Current state": "landed",
            "Next approval required": "none for wiring",
        },
        "claim-store adapter production wiring": {
            "Required evidence": "`Claim store adapter production wiring landing record`",
            "Current state": "landed",
            "Next approval required": "none for wiring",
        },
        "composition-root state binding": {
            "Required evidence": "future explicit composition-root binding approval after production wiring",
            "Current state": "blocked",
            "Next approval required": "binding surface choice",
        },
    }
    assert set(rows) == set(expected_rows)
    for surface, expected_values in expected_rows.items():
        for column, value in expected_values.items():
            assert rows[surface][column] == value
    assert "批准 State ownership split 第十刀: composition-root state binding" in section
    for invariant in (
...
def test_playbook_records_tool_executor_implementation_landing_status_guard() -> None:
    text = _playbook_text()
    section = _heading_section_text(
        text,
    ...
    expected = {
        "landing record": "`ToolExecutor side-effect split implementation landing record`",
        "landing evidence matrix": "`Core implementation landing evidence completeness matrix` ToolExecutor row",
        "sequence gate": "`Core implementation sequence gate` ToolExecutor row",
        "approval evidence": "`ToolExecutor approval transition evidence consistency guard`",
    }
    assert set(rows) == set(expected)
    for surface, location in expected.items():
        assert rows[surface]["Required location"] == location
...
def test_playbook_records_tool_executor_verification_gate_guard() -> None:
    text = _playbook_text()
    section = _heading_section_text(
        text,
    ...
    expected = {
        "red test evidence": "focused ToolExecutor migration test",
        "green focused regression": "executor side-effect characterization and tool-runner adapter tests",
        "legacy executor regression": "executor behavior and cookie injection tests",
        "architecture/source regression": "import layers, source guards, and playbook tests",
        "diff hygiene": "`git diff --check`",
        "post-push status": "`git status --short --branch` plus remote branch SHA",
    }
    assert set(rows) == set(expected)
