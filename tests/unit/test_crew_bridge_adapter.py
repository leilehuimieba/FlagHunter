"""Boundary tests for the crew bridge adapter skeleton."""
...
ADAPTER_PATH = REPO_ROOT / "flaghunter" / "adapters" / "crew" / "crew_bridge_adapter.py"
...
async def test_crew_bridge_adapter_delegates_to_injected_port() -> None:
    module = importlib.import_module("flaghunter.adapters.crew.crew_bridge_adapter")
    package = importlib.import_module("flaghunter.adapters.crew")
    bridge = RecordingCrewBridge()
    adapter = module.CrewBridgeAdapter(bridge)
    request = {
    ...
    receipt = await adapter.dispatch_task(request)

    assert package.CrewBridgeAdapter is module.CrewBridgeAdapter
    assert isinstance(adapter, CrewBridgePort)
...
def test_crew_bridge_adapter_has_no_concrete_or_action_imports() -> None:
